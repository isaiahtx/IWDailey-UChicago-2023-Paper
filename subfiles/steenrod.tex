...
\begin{corollary}\label{E*E_ox_E*E_is_A-graded_pi*S-commutative_ring}
    ...
    constructed in \autoref{Kunneth_map_iso_main} is canonically an $A$-graded $\pi_*(S)$-ring, and sits in the following pushout diagram in $\pi_*(S)\text-\GCA^{A}$:
...
\begin{proof}
	...
	\end{tikzcd}\]
	where the outer arrows are monoid object homomorphisms, thus, we may apply $\pi_*$, which yields the following commutative diagram in $\pi_*(S)\text-\GCA^{A}$ (\autoref{pi_*:CMon_SH-->pi_*(S)-GrCAlg_appendix}):
...
\begin{definition}\label{dual_E-Steenrod_algebra_defn}
    Let $(E,\mu,e)$ be a \emph{commutative} monoid object in $\cSH$ which is flat (\autoref{flat}) and cellular (\autoref{cellular}). Then the \emph{dual $E$-Steenrod algebra} is the pair of $A$-graded abelian groups $(E_*(E),\pi_*(E))$ equipped with the following structure:\begin{enumerate}[label={\arabic*.}]
        ...
        and $\Phi_{E,E}$ is morphism constructed in \autoref{Kunneth_map}, which is proven to be an isomorphism in \autoref{Kunneth_map_iso}, and furthermore an isomorphism in $\pi_*(S)\text-\GCA^{A}$ by \autoref{Phi_E_is_homo_of_A-graded_pi_*S-commutative_rings_main}.
        \item The homomorphism of $A$-graded $\pi_*(S)$-commutative rings
...
\begin{proof}
	...
	\end{tikzcd}\]
	The right region commutes by how $\Psi$ is defined (\autoref{(E,mu,e):Psi_defn_appendix}), and $\Phi_{E,E}$ is an isomorphism, so it suffices to show the left region commutes. To that end, consider the following diagram:
	...
	\todo{finish proof, add reference to that old Adams book}
\end{proof}
