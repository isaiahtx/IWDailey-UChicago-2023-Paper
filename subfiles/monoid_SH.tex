...
In fact, the above definition will be entirely motivated by the work we do here. An interesting fact is that the initial object in the category $\GrCRing^{A}$ is the group algebra $\bZ[A\wedge A]$ on the exterior square $A\wedge A$, viewed as an $A$-graded ring concentrated in degree $0$, and where $\theta_{a,b}$ corresponds to the element $a\wedge b$.\todo{is this actually true?}
...
\begin{definition}\label{R-GrCAlg_defn}
	Given an $A$-graded anticommutative ring $(R,\theta)$ (\autoref{A-graded_anticommutative_ring_defn}), we write $R\text-\GCA^{A}$ to denote the slice category $\GrCRing^{A}/(R,\theta)$. Explicitly:
    \begin{itemize}
