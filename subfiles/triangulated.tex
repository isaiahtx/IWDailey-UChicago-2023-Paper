...
In this appendix, we fix a triangulated category $(\cC,\Sigma,\cD)$ (\autoref{triangulated_defn}). We will denote the hom-group $\cC(X,Y)$ by $[X,Y]$. To start, recall the following definition:
...
%    \[X\xr rY\xr \iota X\]
%    with $r\circ\iota=\id_Y$ and $\iota\circ r=e$. In particular, we may take $Y$ to be the homotopy colimit of
...
\subsection{Adjointly triangulated categories}\label{subsection:adjointly_tri_cat}

From now on, we will assume that $\cC$ is an \emph{adjointly} triangulated category (\autoref{adjointly_triangulated_defn}) with inverse shift $\Omega$, unit $\eta:\Id_\cC\Rightarrow\Sigma\Omega$, and counit $\vare:\Omega\Sigma\Rightarrow\Id_\cC$.
...
In what follows, we fix a tensor triangulated category $(\cC,\otimes,S,\Sigma,e,\cD)$ (\autoref{tentri}).
