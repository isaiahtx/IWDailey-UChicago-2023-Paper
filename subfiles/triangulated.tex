...
\begin{definition}[{\cite[Definition 1.6.4]{Neeman_2001}}]\label{homotopy_colimit_defn}
    ...
    where the shift map $\coprod_{i=0}^\infty X_i\xr{\text{shift}}\coprod_{i=0}^\infty X_i$ is understood to be the direct sum of $j_{i+1}:X_i\to X_{i+1}$, i.e., by the universal property of the coproduct, it is induced by the maps
    \[X_s\xr{j_{s+1}}X_{s+1}\into\coprod_{i=0}^\infty X_i.\]
\end{definition}
...
\begin{definition}\label{homotopy_limit_defn}
    ...
    be a sequence of objects and morphisms in $\cC$. The \emph{homotopy limit} of the sequence, denoted $\holim X_i$, is given (up to non-canonical isomorphism) as the fiber of the map
