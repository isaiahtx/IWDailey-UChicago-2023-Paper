...
\begin{proof}
	...
	On the other hand, suppose that there exists some collection of $a_i\in A$ such that $N$ is a retract of $M:=\bigoplus_i(E\otimes S^{a_i})$ in $E\text-\Mod$:
	% https://q.uiver.app/#q=WzAsMyxbMCwwLCJOIl0sWzEsMCwiTSJdLFsyLDAsIk4iXSxbMCwxLCJcXGlvdGEiLDJdLFsxLDIsInIiLDJdLFswLDIsIiIsMix7ImN1cnZlIjotMywibGV2ZWwiOjIsInN0eWxlIjp7ImhlYWQiOnsibmFtZSI6Im5vbmUifX19XV0=
	\[\begin{tikzcd}
		N & M & N
	...
	\end{tikzcd}\]
	Let $M:=\bigoplus_i(E\otimes S^{a_i})$. Then applying $\pi_*(-)$ to the above diagram yields the following diagram of $A$-graded left $\pi_*(E)$-modules:
	% https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXHBpXyooTikiXSxbMSwwLCJcXHBpXyooTSkiXSxbMiwwLCJcXHBpXyooTikiXSxbMCwxLCJcXHBpXyooXFxpb3RhKSIsMl0sWzEsMiwiXFxwaV8qKHIpIiwyXSxbMCwyLCIiLDIseyJjdXJ2ZSI6LTMsImxldmVsIjoyLCJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJub25lIn19fV1d
	\[\begin{tikzcd}
		{\pi_*(N)} & {\pi_*(M)} & {\pi_*(N)}
		\arrow["{\pi_*(\iota)}"', from=1-1, to=1-2]
		\arrow["{\pi_*(r)}"', from=1-2, to=1-3]
		\arrow[curve={height=-18pt}, Rightarrow, no head, from=1-1, to=1-3]
	\end{tikzcd}\]
	Note we have an isomorphism of $A$-graded left $\pi_*(E)$-modules $\pi_*(M)\xr\cong\bigoplus_i\pi_{*-a_i}(E)$ given by the composition
	\[[S^*,\bigoplus_i(E\otimes S^{a_i})]\xr\cong\bigoplus_i{[S,E\otimes S^{a_i}]}_*\xr{\tau_*}\bigoplus_i{[S,\Sigma^{a_i}E]}_*\xr{\bigoplus_is^{a_i}_{S,E}}\bigoplus_i{[S,E]}_{*-a_i},\]
	where the first isomorphism is because $\pi_*(-)$ is additive\todo{FINISH}
\end{proof}
