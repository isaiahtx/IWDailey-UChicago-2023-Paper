...
\begin{proof}
	...
	\[\begin{tikzcd}
		...
		\arrow["{Z\otimes \mu\otimes W}", from=2-3, to=2-4]
	\end{tikzcd}\]
	The diagram commutes by functoriality of $-\otimes-$, so that diagram (\ref{Phi_Sigma^aZ,W_diagram}) commutes, meaning $\Phi_{\Sigma^aZ,W}$ is an isomorphism as desired. 
...
\begin{proposition}\label{E-module_N_implies_pi*N_is_pi*E_module}
	Let $(E,\mu,e)$ be a monoid object in $\cSH$. Then the assignment $\pi_*:(N,\kappa)\mapsto\pi_*(N)$ yields an additive functor from $E\text-\Mod$ to the category $\pi_*(E)\text-\Mod^A$ of $A$-graded left $\pi_*(E)$-modules and degree-preserving homomorphisms between them, and in fact, it preserves arbitrary coproducts. In particular, if $(N,\kappa)$ is an $E$-module object in $\cSH$, then we view it with its \emph{canonical} $A$-graded left $\pi_*(E)$-module structure given by the graded map
...
\begin{proof}
	...
	Next we claim this preserves arbitrary coproducts. First of all, note that $\pi_*(0)=[S^*,0]=0$ by definition, since $0$ is terminal. Now suppose we have a family of objects $(N_i,\kappa_i)\in E\text-\Mod$ then we would like to show that there is an degree-preserving isomorphism of $A$-graded left $\pi_*(E)$-modules $\bigoplus_i\pi_*(N_i)\xr\cong\pi_*(\bigoplus_iN_i)$ such that the following diagram commutes for all $i$:
	% https://q.uiver.app/#q=WzAsMyxbMSwxLCJcXHBpXyooXFxiaWdvcGx1c19pTl9pKSJdLFswLDAsIlxccGlfKihOX2kpIl0sWzAsMSwiXFxiaWdvcGx1c19pXFxwaV8qKE5faSkiXSxbMSwwLCJcXHBpXyooXFxpb3RhX3tOX2l9KSJdLFsyLDAsIlxcY29uZyJdLFsxLDIsIlxcaW90YV97XFxwaV8qKE5faSl9IiwyXV0=
	\begin{equation}\label{diagram:pi_*_preserves_arbitrary_coproducts}\begin{tikzcd}
		{\pi_*(N_i)} \\
		{\bigoplus_i\pi_*(N_i)} & {\pi_*(\bigoplus_iN_i)}
		\arrow["{\pi_*(\iota_{N_i})}", from=1-1, to=2-2]
		\arrow["\cong", from=2-1, to=2-2]
		\arrow["{\iota_{\pi_*(N_i)}}"', from=1-1, to=2-1]
	\end{tikzcd}\end{equation}
	First of all, since each $S^a$ is compact, for all $a\in A$ we have isomorphisms
	\[\bigoplus_i\pi_a(N_i)=\bigoplus_i[S^a,N_i]\xr\cong[S^a,\bigoplus_iN_i]=\pi_a(\bigoplus_iN_i),\]
	and these combine together to yield $A$-graded isomorphisms $q_{\{N_i\}}:\bigoplus_i\pi_*(N_i)\xr\cong\pi_*(\bigoplus_iN_i)$. Explicitly unravelling defintions, the above maps send a generator $x:S^a\to N_i$ in $\pi_a(N_i)$ to the class $S^a\xr xN_i\xr{\iota_{N_i}}\bigoplus_iN_i$. To see this isomorphism is further an isomorphism of left $\pi_*(E)$-modules, it suffices to show that given a generator $r:S^a\to E$ in $\pi_*(E)$ and a generator $x:S^b\to N_i$ in $\pi_b(N_i)\leq\bigoplus_i\pi_*(N_i)$, that $r\cdot q_{\{N_i\}}(x)=q_{\{N_i\}}(r\cdot x)$. To that end, consider the following diagram:
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{S^{a+b}} & {S^a\otimes S^b} & {E\otimes N_i} && {E\otimes\bigoplus_iN_i} \\
		&&&& {\bigoplus_i(E\otimes N_i)} \\
		&& {N_i} && {\bigoplus_iN_i}
		\arrow["{\phi_{a,b}}", from=1-1, to=1-2]
		\arrow["{r\otimes x}", from=1-2, to=1-3]
		\arrow["{E\otimes\iota_{N_i}}", from=1-3, to=1-5]
		\arrow["{\kappa_i}", from=1-3, to=3-3]
		\arrow["{\iota_{N_i}}", from=3-3, to=3-5]
		\arrow["\cong", from=1-5, to=2-5]
		\arrow["{\bigoplus_i\kappa_i}", from=2-5, to=3-5]
	\end{tikzcd}\]
	Unravelling definitions, the top composition is $r\cdot q_{\{N_i\}}(x)$, while the bottom composition is $q_{\{N_i\}}(r\cdot x)$. In \autoref{coproduct_of_E_modules_is_coproduct_in_E_mod}, we showed that $\iota_{N_i}:N_i\into\bigoplus_iN_i$ is an $E$-module object homomorphism, and the right map is precisely the action map for $\bigoplus_iN_i$ as an $E$-module object, so the diagram does indeed commute --- $q_{\{N_i\}}$ is a degree-preserving isomorphism of $A$-graded left $\pi_*(E)$-module objects, as desired. Finally, to see diagram (\ref{diagram:pi_*_preserves_arbitrary_coproducts}) commutes, observe that by unravelling definitions, given a homogeneous element $x:S^a\to N_i$ in $\pi_*(N_i)$, chasing it either way around the diagram yields the composition
	\[S^a\xr xN_i\xr{\iota_{N_i}}\bigoplus_i N_i,\]
	so that diagram (\ref{diagram:pi_*_preserves_arbitrary_coproducts}) commutes for generators, and thus commutes entirely, since all the maps involved are homomorphisms.
\end{proof}
...
\begin{proof}
	...
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{E\otimes S^a\otimes\bigoplus_iN_i} && {E\otimes\bigoplus_i(S^a\otimes N_i)} \\
		{S^a\otimes E\otimes\bigoplus_iN_i} && {\bigoplus_i(E\otimes S^a\otimes N_i)} \\
		{S^a\otimes\bigoplus_i(E\otimes N_i)} && {\bigoplus_i(S^a\otimes E\otimes N_i)} \\
		{S^a\otimes\bigoplus_iN_i} && {\bigoplus_i(S^a\otimes N_i)}
		\arrow["E\otimes\cong", from=1-1, to=1-3]
		\arrow["{\tau\otimes\bigoplus_iN_i}"', from=1-1, to=2-1]
		\arrow["{S^a\otimes\cong}"', from=2-1, to=3-1]
		\arrow["{S^a\otimes\bigoplus_i\kappa_i}"', from=3-1, to=4-1]
		\arrow["\cong", from=1-3, to=2-3]
		\arrow["{\bigoplus_i(\tau\otimes N_i)}", from=2-3, to=3-3]
		\arrow["{\bigoplus_i(S^a\otimes\kappa_i)}", from=3-3, to=4-3]
		\arrow["\cong", from=4-1, to=4-3]
		\arrow["\cong", from=3-1, to=3-3]
		\arrow["\cong"{description}, from=1-1, to=2-3]
	\end{tikzcd}\]
	The top region comutes by additivity of $E\otimes-$. The bottom two region commute by naturality of the additivity isomorphisms for $-\otimes-$.
\end{proof}
...
\begin{theorem}\label{theorem:UCT}
    ...
    \[[X,E\otimes Y]\to\Hom_{\pi_*(E)}(E_*(X),E_*(Y)),\qquad[X\xr fE\otimes Y]\mapsto [\pi_*(\mu\otimes Y)\circ E_*(f)]\]
...
i.e., the $E$-cohomology of $X$ is isomorphic to the dual of the $E$-homology of $X$ when $E_*(X)$ is a graded projective module. Hence why we call it the universal coefficient theorem. The condition that $E_*(X)$ be graded projective is a rather technical one, although thankfully it is often satisfied in practice. In the case $\pi_*(E)$ is a field, or more generally a product of fields, every $\pi_*(E)$-module is projective and this is trivially satisified. Alternatively, if the objects $N:=E\otimes X$ and $E$ are cellular, then it is satisfied precisely when $N$ is a retract of a direct sum of suspensions of copies of $E$ via $E$-module homomorphisms:

\begin{proposition}\label{if_pi_*N_graded_proj_then_retract_of_wedge_of_susps}
	Let $(E,\mu,e)$ be a monoid object and $(N,\kappa)$ an $E$-module object in $\cSH$. If there exists a collection of $a_i\in A$ such that $N$ is a retract of $\bigoplus_i(E\otimes S^{a_i})$ in $E\text-\Mod$,\footnote{Here $\bigoplus_i(E\otimes S^{a_i})$ is a coproduct (\autoref{coproduct_of_E_modules_is_coproduct_in_E_mod}) of a bunch of free $E$-module objects (\autoref{free_forgetful_E-Mod}), so it is itself an $E$-module object.} then $\pi_*(N)$ is a graded projective (\autoref{graded_projective_module}) $\pi_*(E)$-module. If $E$ and $N$ are cellular, then the converse holds as well.
\end{proposition}
\begin{proof}
	First suppose that there exists some collection of $a_i\in A$ such that $N$ is a retract of $M:=\bigoplus_i(E\otimes S^{a_i})$ in $E\text-\Mod$:
	% https://q.uiver.app/#q=WzAsMyxbMCwwLCJOIl0sWzEsMCwiTSJdLFsyLDAsIk4iXSxbMCwxXSxbMSwyXSxbMCwyLCIiLDIseyJjdXJ2ZSI6LTMsImxldmVsIjoyLCJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJub25lIn19fV1d
	\[\begin{tikzcd}
		N & M & N
	...
	\end{tikzcd}\]
	Then applying $\pi_*(-)$ to the above diagram yields the following diagram of $A$-graded left $\pi_*(E)$-modules:
	% https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXHBpXyooTikiXSxbMSwwLCJcXHBpXyooTSkiXSxbMiwwLCJcXHBpXyooTikiXSxbMCwxXSxbMSwyXSxbMCwyLCIiLDIseyJjdXJ2ZSI6LTMsImxldmVsIjoyLCJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJub25lIn19fV1d
	\[\begin{tikzcd}
		{\pi_*(N)} & {\pi_*(M)} & {\pi_*(N)}
		\arrow[from=1-1, to=1-2]
		\arrow[from=1-2, to=1-3]
		\arrow[curve={height=-18pt}, Rightarrow, no head, from=1-1, to=1-3]
	\end{tikzcd}\]
	Note we have an isomorphism of $A$-graded left $\pi_*(E)$-modules $\pi_*(M)\xr\cong\bigoplus_i\pi_{*-a_i}(E)$ given by the composition
	\[\pi_*(\bigoplus_i(E\otimes S^{a_i}))\xr\cong\bigoplus_iE_*(S^{a_i})=\bigoplus_iE_*(\Sigma^{a_i}S)\xr{\bigoplus_it^{a_i}_S}\bigoplus_iE_{*-a_i}(S)=\bigoplus_i\pi_{*-a_i}(E),\]
	where the first isomorphism is because $\pi_*:E\text-\Mod\to\pi_*(E)\text-\Mod^A$ preserves arbitrary coproducts (by \autoref{E-module_N_implies_pi*N_is_pi*E_module}), the maps $t^{a_i}_{S}:E_*(\Sigma^{a_i}S)\xr\cong E_{*-a_i}(S)$ are the degree-preserving isomorphisms of $A$-graded left $\pi_*(E)$-modules from \autoref{t^a_isos_are_E_*E-comodule_isos}, and the equalities follow from the coherence theorem for monoidal categories, which tells us we may assume $S\otimes-=-\otimes S=\mathrm{Id}_{\cSH}$. Hence $\pi_*(N)$ is isomorphic in $\pi_*(E)\text-\Mod^A$ to a free $\pi_*(E)$ module, so that $\pi_*(N)$ is a retract in $\pi_*(E)\text-\Mod^A$ of a free $\pi_*(E)$-module, meaning it is graded projective, as desired.

	On the other hand, suppose that $E$ and $N$ are cellular and $\pi_*(N)$ is graded projective, and pick some homogeneous generating set $\{x_i\}\sseq\pi_*(N)$. Let $M:=\bigoplus_i(E\otimes S^{|x_i|})$. We have a map
	\[r:M\to N\]
	induced by the maps
	...
	and again since $\pi_*$ is an isomorphism here, we have that $\ell\circ\ell=\ell$, so that $\ell$ is an idempotent in $\cSH$. By \autoref{cellular_idempotent_splits_cellularly}, every idempotent in $\cSH$ splits, meaning $\ell$ factors in $\cSH$ as
	\[\ell:M\xr{r'}X\xr{\iota'}M\]
	with $r'\circ \iota'=\id_X$. Since $X$ is a retract of an $E$-module object, and the corresponding idempotent is an $E$-module homomorphism, it follows purely formally that $X$ may be canonically viewed as an $E$-module object, and that $r':M\to X$ and $\iota':X\to M$ are homomorphisms of $E$-module objects (see \autoref{retract_of_module_whose_idempotent_is_module_homomorphism_is_module} for details). Note that since $E$ and each $S^{|x_i|}$ are cellular, $E\otimes S^{|x_i|}$ is cellular for all $i\in I$ (by \autoref{cellular_closed_under_tensor}), so that $M=\bigoplus_i(E\otimes S^{|x_i|})$ is cellular, as by definition an arbitrary coproduct of cellular objects is cellular. Thus since $X$ is a retract of a cellular object in a triangulated category with arbitrary coproducts, it follows that $X$ is cellular as well (see \autoref{cellular_idempotent_splits_cellularly} for details). Now consider the following commutative diagram
...
It turns out that the condition that $N$ is a retract of a direct sum of suspensions of $E$ is really the key condition which allows the universal coefficient theorem to go through, as the following two propositions illustrate.
...
\begin{proof}
    ...
    \end{tikzcd}\]
	Here the top left vertical isomorphism exibits the universal property of the coproduct in $E\text-\Mod$, and middle left vertical isomorphism below that is the free-forgetful adjunction for $E$-modules (\autoref{free_forgetful_E-Mod}). The bottom horizontal isomorphism is the product of the evaluation-at-$1$ isomorphisms (\autoref{ev_at_1_is_iso}). On the other side, the top right vertical isomorphism is given by the fact that $\pi_*:E\text-\Mod\to\pi_*(E)\text-\Mod^A$ preserves arbitrary coproducts (by \autoref{E-module_N_implies_pi*N_is_pi*E_module}). The middle right vertical isomorphism exhibits the universal property of the coproduct of modules. Finally the bottom right vertical isomorphism is given by the composition
	\[\pi_*(E\otimes S^{a_i})=E_*(S^{a_i})=E_*(\Sigma^{a_i}S)\xr{t^{a_i}_S}E_{*-a_i}(S)=\pi_{*-a_i}(E),\]
	where $t^{a_i}_X$ is the $A$-graded isomorphism of left $\pi_*(E)$-modules from \autoref{t^a_isos_are_E_*E-comodule_isos}, and the second and last equalities follow by the coherence theorem for monoidal categories, which tells us we may assume $S\otimes-$ and $-\otimes S$ are the identity on $\cSH$. Now, we claim this diagram commutes. This really simply amounts to unravelling definitions, and chasing a homomorphism $f:\bigoplus_i(E\otimes S^{a_i})\to N'$ of $E$-module objects both ways around the diagram yields the composition
...
\begin{proposition}\label{UCT_for_retract}
	...
	\[[X,N]\to\Hom_{\pi_*(E)}(E_*(X),\pi_*(N)),\qquad [X\xr fN]\mapsto[\pi_*(\kappa)\circ E_*(f)]\]
