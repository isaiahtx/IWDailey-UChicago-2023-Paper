...
\subsection{Modules over a monoid object}
...
\begin{definition}
	Given a monoid object $(E,\mu,e)$ in $\cC$, we write $E\text-\Mod$ to denote the category of left module objects over $E$ and left $E$-module homomorphisms between them. We denote the homset in $E\text-\Mod$ by
...
\begin{proposition}\label{Kunneth_map}
    ...
    (where here we are considering the canonical $A$-graded right $\pi_*(E)$-module structure on $\pi_*(Z\otimes E)=Z_*(E)$ and the canonical left $A$-graded $\pi_*(E)$-module structure on $\pi_*(E\otimes W)=E_*(W)$ given in \autoref{module}, so that $\pi_*(Z\otimes E)\otimes_{\pi_*(E)}\pi_*(E\otimes W)$ is a well-defined $A$-graded abelian group by \autoref{tensor_of_A_graded_is_A_graded}). Furthermore, this homomorphism is natural in both $Z$ and $W$.
\end{proposition}
\begin{proof}
	First, recall by definition of the tensor product, in order to show the assignment $\pi_*(Z\otimes E)\times\pi_*(E\otimes W)\to\pi_*(Z\otimes E\otimes W)$ induces a homomorphism $\pi_*(Z\otimes E)\otimes_{\pi_*(E)}\pi_*(E\otimes W)\to\pi_*(Z\otimes E\otimes W)$ of $A$-graded abelian groups, it suffices to show that the assignment is $\pi_*(E)$-balanced, i.e., that it is linear in each argument and satisfies $zr\cdot w=z\cdot rw$ for $z\in \pi_*(Z\otimes E)$, $w\in\pi_*(E\otimes W)$, and $r\in\pi_*(E)$.
	
	First, note that by \autoref{bilinear} it is straightforward to see that the assignment commutes with addition of maps in each argument. Now, let $a,b,c\in A$, $z:S^a\to Z\otimes E$, $w:S^b\to E\otimes W$, and $r:S^c\to E$. Then we wish to show $z r\cdot w=z\cdot rw$. Consider the following diagram (where here we are passing to a symmetric strict monoidal category):
...
\begin{lemma}\label{E_homology_suspension_iso_t^a's_appendix}
	...
	(where here we are ignoring associators and unitors).
	Furthermore this isomorphism is natural in $X$, and if $E$ is a monoid object in $\cSH$ then it is a natural isomorphism of $\pi_*(E)$-modules.
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
	First to see $\Phi_{Z,\Sigma^aW}$ is an isomorphism, consider the following diagram
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{Z_*(E)\otimes_{\pi_*(E)}E_*(\Sigma^aW)} && {Z_*(E)\otimes_{\pi_*(E)}E_{*-a}(W)} \\
		\\
		{\pi_*(Z\otimes E\otimes\Sigma^aW)=(Z\otimes E)_*(\Sigma^aW)} && {(Z\otimes E)_{*-a}(W)=Z\pi_{*-a}(Z\otimes E\otimes W)}
		\arrow["{Z_*(E)\otimes_{\pi_*(E)}t_a^W}", from=1-1, to=1-3]
		\arrow["{\Phi_{Z,W}}", from=1-3, to=3-3]
		\arrow["{\Phi_{Z,\Sigma^aW}}"', from=1-1, to=3-1]
		\arrow["{t_a^W}", from=3-1, to=3-3]
	\end{tikzcd}\]
	where the maps $t_a$ are defined in \autoref{E_homology_suspension_iso_t^a's_appendix}. The top arrow is well-defined since $t_a^W:E_*(\Sigma^aW)\to E_{*-a}(W)$ is a degree $-a$ isomorphism of left $\pi_*(E)$-modules by the aforementioned lemma. In order to show the left vertical arrow is an isomorphism, it suffices to show the diagram commutes, as all the other arrows are isomorphisms. To see this, note it suffices to chase a homogeneous pure tensor around the diagram, as all the maps here are homomorphisms. So let $x:S^b\to Z\otimes E$ in $Z_*(E)$ and $y:S^c\to E\otimes S^a\otimes W$ in $E_*(\Sigma^aW)$, and consider the following diagram exhibiting the two ways to chase $x\otimes y$ around:
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{S^{b+c-a}} && {Z\otimes E\otimes E\otimes S^a\otimes S^{-a}\otimes W} && {Z\otimes E\otimes E\otimes W} \\
		\\
		{S^b\otimes S^c\otimes S^{-a}} && {Z\otimes E\otimes E\otimes S^a\otimes W\otimes S^{-a}} && {Z\otimes E\otimes W} \\
		\\
		&& {Z\otimes E\otimes S^a\otimes W\otimes S^{-a}} && {Z\otimes E\otimes S^a\otimes S^{-a}\otimes W}
		\arrow["\phi", from=1-1, to=3-1]
		\arrow["{x\otimes y\otimes S^{-a}}", from=3-1, to=3-3]
		\arrow["{Z\otimes E\otimes E\otimes S^a\otimes \tau}", from=3-3, to=1-3]
		\arrow["{Z\otimes E\otimes E\otimes \phi_{a,-a}^{-1}\otimes W}", from=1-3, to=1-5]
		\arrow["{Z\otimes \mu\otimes W}", from=1-5, to=3-5]
		\arrow["{Z\otimes \mu\otimes S^a\otimes W\otimes S^{-a}}"', from=3-3, to=5-3]
		\arrow["{Z\otimes E\otimes S^a\otimes \tau}"', from=5-3, to=5-5]
		\arrow["{Z\otimes E\otimes \phi_{a,-a}^{-1}\otimes W}"', from=5-5, to=3-5]
		\arrow["{Z\otimes \mu\otimes S^a\otimes S^{-a}\otimes W}", from=1-3, to=5-5]
	\end{tikzcd}\]
	Each triangle commutes by functoriality of $-\otimes-$, so the diagram commutes as desired. Thus, we've shown $\Phi_{Z,\Sigma^aW}$ is an isomorphism for all $a\in A$.

	On the other hand, in order to see $\Phi_{\Sigma^aZ,W}$ is an isomorphism, consider the following diagram:
