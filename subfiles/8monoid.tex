...
\begin{proposition}
	Let $(E,\mu,e)$ be a \emph{flat} monoid object in $\cSH$ (\autoref{flat}) and let $X$ be any \emph{cellular} object in $\cSH$ (\autoref{cellular}). Then the natural homomorphism
...
%	\[E_*(E)\otimes_{\pi_*(E)}E_*(S^a)\cong E_*(E)\otimes_{\pi_*(E)}\pi_{*-a}(E)\cong E_{*-a}(E)\cong E_*(E\otimes S^a).\]
	...
	if two of three of the objects $X$, $Y$, and $Z$ belong to $\cE$, then so does the third. Indeed, supposing this is true, note first of all that since $\cSH$ is tensor triangulated, by axiom TT3 (\autoref{tentri}), the following triangle is also distinguished:
