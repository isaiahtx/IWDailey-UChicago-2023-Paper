...
From now on we fix a monoidal closed tensor triangulated category $(\cSH,\otimes,S,\Sigma,e,\cD)$ (\autoref{tentri}) with arbitrary (small) (co)products and sub-Picard grading $(A,\1,h,\{S^a\},\nu,\{\phi_{a,b}\})$ (\autoref{sub_Picard_grading_defn}), and we adopt the conventions outlined in \Cref{setup}. In all proofs that follow we will freely use the coherence theorem for symmetric monoidal categories. In particular, we will assume without loss of generality that the associators and unitors in $\cSH$ are identities.
...
\begin{lemma}
	Let $E$ and $X$ be objects in $\cSH$. Then for all $a\in A$, there is an $A$-graded isomorphism of left $\pi_*(E)$-modules
	...
	(where here we are ignoring associators and unitors). Furthermore this isomorphism is natural in $X$.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{t's_commute_with_Phi's}
	Given a monoid object $(E,\mu,e)$ in $\cSH$, the maps $\Phi_X$ constructed in \autoref{Kunneth_map} commute with the natural isomorphisms $t^a_X$, in the sense that the following diagram commutes for all $a\in A$ and $X$ in $\cSH$:
...
\begin{corollary}\label{t_commutes_with_Phi's_corollary}
	...
	\[E_*(\Sigma X)\xr{E_*(\nu_X^{-1})}E_*(\Sigma^1 X)\xr{t^\1_X}E_{*-\1}(X).\]
...
%	\[E_*(E)\otimes_{\pi_*(E)}E_*(S^a)\cong E_*(E)\otimes_{\pi_*(E)}\pi_{*-a}(E)\cong E_{*-a}(E)\cong E_*(E\otimes S^a).\]
	...
	Finally, it remains to show that $\cE$ is closed under arbitrary products. Let $\{X_i\}_{i\in I}$ be a collection of objects in $\cE$ indexed by some (small) set $I$. Then we'd like to show that $X:=\coprod_iX_i$ belongs to $\cE$. First of all, note that $E\otimes-$ preserves arbitrary coproducts, as it has a right adjoint $F(E,-)$. Thus without loss of generality we may take $\coprod_iE\otimes X_i=E\otimes\coprod_iX_i$ (as $E\otimes\coprod_iX_i$ \emph{is} a coproduct of all the $E\otimes X_i$'s). Now, recall that we have chosen each $S^a$ to be a compact object (\autoref{defn_compact}), so that the canonical map
	\[s:\coprod_i E_*(X_i)=\coprod_i[S^*,E\otimes X_i]\to[S^*,\coprod_iE\otimes X_i]=[S^*,E\otimes X]=E_*(X)\]
	is an isomorphism, natural in $X_i$ for each $i$. Note in particular that it is an isomorphism of left $\pi_*(E)$-modules. To see this, first note it suffices to check that $s(r\cdot x)=r\cdot s(x)$ for some homogeneous $x\in E_*(X_i)$ for some $i$, as such $x$ generate $\coprod_i E_*(X_i)$ by definition, and $s$ is a homomorphism of abelian groups. Then given $r:S^a\to E\otimes E$ and $x:S^b\to E\otimes X_i$, consider the following diagram	
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{S^{a+b}} & {S^a\otimes S^b} & {E\otimes E\otimes E\otimes X_i} & {E\otimes E\otimes \coprod_i(E\otimes X_i)} \\
		&&& {E\otimes E\otimes E\otimes X} \\
		&&& {E\otimes E\otimes X} \\
		&& {E\otimes E\otimes X_i} & {E\otimes \coprod_i(E\otimes X_i)}
		\arrow["{\phi_{a,b}}", from=1-1, to=1-2]
		\arrow["{x\otimes y}", from=1-2, to=1-3]
	...
	\end{tikzcd}\]
	where $\iota_{E\otimes X_i}:E\otimes X_i\into\coprod_i(E\otimes X_i)$ and $\iota_{X_i}:X_i\into\coprod_iX_i$ are the maps determined by universal property of the coproduct. Commutativity of the two triangles is again by the fact that $E\otimes-$ is colimit preserving. Commutativity of the trapezoid is functoriality of $-\otimes-$. Thus, the top arrow in the following diagram is well-defined:
	% https://q.uiver.app/#q=
	\begin{equation}\label{kunneth_iso_pt_3_diag}\begin{tikzcd}
		{\coprod_iE_*(E)\otimes_{\pi_*(E)}E_*(X_i)} & {E_*(E)\otimes_{\pi_*(E)}\coprod_iE_*(X_i)} & {E_*(E)\otimes_{\pi_*(E)} E_*(X)} \\
		\\
		{\coprod_iE_*(E\otimes X_i)} & {E_*(\coprod_iE\otimes X_i)} & {E_*(E\otimes X)}
		\arrow["{E_*(E)\otimes_{\pi_*(E)} s}", from=1-2, to=1-3]
		\arrow["{\Phi_X}", from=1-3, to=3-3]
		\arrow[Rightarrow, no head, from=1-1, to=1-2]
		\arrow["{\coprod_i\Phi_{X_i}}"', from=1-1, to=3-1]
	...
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{S^{a+b}} & {S^a\otimes S^b} & {E\otimes E\otimes E\otimes X_i} & {E\otimes E\otimes \coprod_iE\otimes X_i} \\
		&& {E\otimes E\otimes X_i} & {E\otimes E\otimes E\otimes X} \\
		&& {\coprod_iE\otimes E\otimes X_i} & {E\otimes E\otimes X}
		\arrow["{\phi_{a,b}}", from=1-1, to=1-2]
		\arrow["{x\otimes y}", from=1-2, to=1-3]
		\arrow["{E\otimes E\otimes \iota_{E\otimes X_i}}", from=1-3, to=1-4]
