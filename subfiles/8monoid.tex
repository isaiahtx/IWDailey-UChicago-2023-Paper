...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item The right arrow is that given by \autoref{tensor_shift_A_graded},
		\item The bottom arrow is the isomorphism
		% https://q.uiver.app/#q=
		\[\begin{tikzcd}
			{E_{*-a}(E)=[S^{*-a},E\otimes E]} & {[S^{-a}\otimes S^*,E\otimes E]} & {[S^*,S^a\otimes E\otimes E]} \\
			& {[S^*,E\otimes S^a\otimes E]} & {[S^*,E\otimes E\otimes S^a]=E_*(E\otimes S^a)}
			\arrow["{(\phi_{-a,*}^{-1})^*}", from=1-1, to=1-2]
			\arrow["\cong", from=1-2, to=1-3]
			\arrow["{(\tau\otimes E)_*}"', from=1-3, to=2-2]
			\arrow["{(E\otimes\tau)_*}", from=2-2, to=2-3]
		\end{tikzcd}\]
		where again the middle arrow is the adjunction given by \autoref{Sigma^a,Sigma^-a_adjoint_equiv}. Now, we wish to show diagram (\ref{deq2}) commutes.\todo{finish}
	\end{enumerate} 
\end{proof}
