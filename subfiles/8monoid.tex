...
\end{proposition}
\begin{proof}
	\todo{rewrite proof in a permutative category}
	...
	\end{tikzcd}\]
	The left square commutes by naturality of $\lambda$. The right square commutes by functoriality of $-\otimes-$. Hence $\eta$ is indeed a natural isomorphism.
...
%	\[E_*(E)\otimes_{\pi_*(E)}E_*(S^a)\cong E_*(E)\otimes_{\pi_*(E)}\pi_{*-a}(E)\cong E_{*-a}(E)\cong E_*(E\otimes S^a).\]
	...
	if two of three of the objects $X$, $Y$, and $Z$ belong to $\cE$, then so does the third. 
	First we claim that if $X$ belongs to $\cE$, then for any $a\in A$, the object $\Sigma^aX=S^a\otimes X$ also belongs to $\cE$. To see this, consider the following diagram
