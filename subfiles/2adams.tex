...
\begin{definition}\label{stable_homotopy_cat_defn}
	...
	\begin{itemize}
		\item An additive closed symmetric monoidal category $(\cSH,\otimes,S)$ with arbitrary (small) (co)products.
		\item A pointed abelian group $(A,\1)$ along with a homomorphism $h:A\to\Pic\cSH$.
		\item For each $a\in A$, a chosen representative $S^a$ in the isomorphism class $h(a)$ such that $S^0=S$.
		\item For each $a,b\in A$, an isomorphism $\phi_{a,b}:S^{a+b}\to S^a\otimes S^b$. 
		\item A collection $\cD$ of triangles in $\cC$ of the following form
		\[X\to Y\to Z\to \Sigma X.\]
	\end{itemize}
	These data are subject to the following additional conditions. First, we require that the $\phi_{a,b}$'s are \emph{coherent}, in the following sense:
	\begin{itemize}
		\item For all $a\in A$, we must have $\phi_{a,0}=\rho_{S^a}^{-1}:S^{a}\to S^a\otimes S$ and $\phi_{0,a}=\lambda_{S^a}^{-1}:S^a\to S\otimes S^a$.
		...
		\end{tikzcd}\]
	\end{itemize}
	Now, given $a\in A$, define $\Sigma^a:=S^a\otimes-$ and $\Omega^a:=\Sigma^{-a}=S^{-a}\otimes-$. In particular, let $\Sigma:=\Sigma^\1$ and $\Omega:=\Omega^\1$. Then the above two conditions give that $\Sigma^a$ and $\Omega^a$ form an adjoint equivalence of $\cSH$, with unit $\eta^a$ and counit $\vare^a$ defined by
	\[\eta^a_X:X\xrightarrow\cong S\otimes X\xrightarrow{\phi_{-a,a}\otimes X}(S^{-a}\otimes S^a)\otimes X\xrightarrow\cong S^{-a}\otimes (S^a\otimes X)=\Omega^{a}\Sigma^a X\]
	and
	\[\vare^a_X:\Sigma^a\Omega^{a}X=S^a\otimes (S^{-a}\otimes X)\xrightarrow{\cong}(S^a\otimes S^{-a})\otimes X\xrightarrow{\phi_{a,-a}^{-1}\otimes X}S\otimes X\xrightarrow\cong X.\]
	Thus, further require the following:
	\begin{itemize}
		\item $(\cSH,\Sigma,\Omega,\eta^\1,\vare^\1,\cD)$  is a tensor triangulated category, where the isomorphisms $\Sigma X\otimes Y\cong\Sigma(X\otimes Y)$ are given by the associator:
		\[\Sigma X\otimes Y=(S^\1\otimes X)\otimes Y\xrightarrow{\cong}S^\1\otimes(X\otimes Y)=\Sigma(X\otimes Y).\]
		(See \autoref{triangulated_defn} for the definition of a triangulated category, and \autoref{tentri} for the definition of a tensor triangulated category.)
	\end{itemize}
\end{definition}

From now on we fix the data given in the above definition, and we establish the following conventions:
...
\begin{proposition}\label{Sigma^a,Sigma^-a_adjoint_equiv}
	For each $a\in A$, the isomorphisms $\eta^a_X:X\to\Omega^{a}\Sigma^a X$ and $\vare^a_X:\Sigma^a\Omega^a X\to X$ defined in \autoref{stable_homotopy_cat_defn} exhibit an adjoint autoequivalence $(\Sigma^a,\Omega^a,\eta^a,\vare^a)$ of $\cSH$.
\end{proposition}
\begin{proof}
	...
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{\Sigma^a} & {\Sigma^a\Omega^a\Sigma^a} & {\Omega^a\Sigma^a\Omega^a} & {\Omega^a} \\
		& {\Sigma^a} & {\Omega^a}
		\arrow["{\Sigma^a\eta^a}", from=1-1, to=1-2]
		\arrow["{\varepsilon^a\Sigma^a}", from=1-2, to=2-2]
		\arrow["{\Omega^a\varepsilon^a}"', from=1-3, to=2-3]
		\arrow[Rightarrow, no head, from=1-4, to=2-3]
		\arrow[Rightarrow, no head, from=1-1, to=2-2]
		\arrow["{\eta^a\Omega^a}"', from=1-4, to=1-3]
	\end{tikzcd}\]
	...
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{S^a\otimes X} && {S^a\otimes S^{-a}\otimes S^a\otimes X} \\
		\\
		&& {S^a\otimes X}
		\arrow["{S^a\otimes\phi_{-a,a}\otimes X}", from=1-1, to=1-3]
		\arrow["{\phi_{a,-a}^{-1}\otimes S^a\otimes X}", from=1-3, to=3-3]
