...
Finally, we may construct the spectral sequence. Henceforth, we will assume the reader is familiar with the theory of spectral sequenes arising from unrolled exact couples, along with the notion of (conditional, strong) convergence of such spectral sequences to their (co)limits. The primary reference for these facts will be Boardman's paper \cite{Boardman_1999} on conditionally convergent spectral sequences. When using any results from this reference, we will be sure to provide a proper citation. Note that Boardman works with $\bZ$-graded groups, although everything he does carries through entirely the same with $A$-graded groups.

From now on, let $(E,\mu,e)$ be a monoid object and $X$ and $Y$ be objects in $\cSH$.
...
\begin{definition}\label{ASS}
	...
	\[\begin{tikzcd}
		...
		\arrow["{k_0}"{pos=0.3}, dashed, from=2-5, to=1-4]
	\end{tikzcd}\]
	We can extend this diagram to the right by setting $Y_s=Y$, $W_s=0$, and $i_s=\id_Y$ for $s<0$. Then we may apply the functor ${[X,-]}_\ast$, and by \autoref{X,Y*_LES_compact}, we obtain the following $A$-graded unrolled exact couple:
	...
	In \cite[\S0]{Boardman_1999}, it is described how we may associate a $\bZ\times A$-graded spectral sequence $r\mapsto(E_r^{*,*}(X,Y),d_r)$ to the above $A$-graded unrolled exact couple, where $d_r$ has $\bZ\times A$-degree $(r,-\1)$. We call this spectral sequence the \emph{$E$-Adams spectral sequence for the computation of ${[X,Y]}_*$}.
\end{definition}

For those who would rather not lose themselves in Boardman's document, we give a brief unravelling of how it applies to the present situation. Given some $s\in\bZ$ and some $r\geq1$, we may define the following $A$-graded subgroups of ${[X,W_s]}_*$:
...
In the case $r=1$, since $B_1^s=0$ and $Z_1^s={[X,W_s]}_*$, we have that $E_1^s={[X,W_s]}_*$, and given some $x\in E_1^s=[X,W_s]_*$, the differential $d_1$ is given by $d_1(x)=j_{s+1}(\partial_s(x))$, so that $d_1=j\circ\partial$. Furthermore, since the unrolled exact couple which yields the spectral sequence vanishes on its negative terms, we have that $E_r^{s,a}(X,Y)=0$ for $s<0$. In particular, the $E$-Adams spectral sequence is a \emph{half-plane spectral sequence with entering differentials}, in the sense of \cite[\S7]{Boardman_1999}.

Showing in explicit detail that all of these definitions make sense and are well-defined is relatively straightforward. Furthermore, one may check that that $d_r\circ d_r=0$, and that
\[\ker d_r^s/\imm d_r^s=\frac{Z_{r+1}^s/B_r^s}{B_{r+1}^s/B_r^s}\cong Z_{r+1}^s/B_{r+1}^s=E_{r+1}^s.\]

Above we constructed the spectral sequence by means of the ``canonical'' $E$-Adams resolution of $Y$, but one may more generally pursue the notion of \emph{$E$-Adams resolutions} of the object $Y$, for which the canonical Adams resolution constructed above will be an example. We do not explore this generality here (although one certainly could); these are useful when one wants to construct an Adams resolution from an algebraic resolution of $E_*(Y)$, or by modifying an Adams resolution for some other object. One may find different notions of what exactly constitutes an Adams resolution in the literature (for example, see \cite[Definition 2.2.1]{RGB} or \cite[Definition 11.3.1]{Rognes_SSeq}), and they will always be defined so that the $E$-Adams spectral sequence for ${[X,Y]}_*$ is independent of the choice of Adams resolution for $Y$, at least from its $E_2$ page onwards. One important condition (or definitional consequence) one will always find for an $E$-Adams resolution is that the $i$'s must vanish in $E$-homology. We can show that the canonical $E$-Adams resolution we have constructed satisfies this property:

\begin{lemma}\label{canonical_E-Adams_resolution_is_an_Adams_resolution}
	Let $i_s$ and $j_s$ be as in \autoref{defn:canonical_E-Adams_resolution}. Then the maps $j_s:Y_s\to W_s$ induce split monomorphisms $E_*(j_s)$ on $E$-homology, so that in particular the maps $i_s:Y_{s+1}\to Y_s$ vanish in $E$-homology, i.e., $E_*(i_s)$ is the zero map.
\end{lemma}
\begin{proof}
...
\begin{theorem}\label{E_2_page_characterization}
...
As a result of this theorem, the spectral sequence is often shifted, by re-defining 
\[E_r^{s,a}(X,Y)^\text{new}:=E_r^{s,a+\mbf s}(X,Y),\]
in which case the given isomorphism characterizing the $E_2$ page is strictly degree-preserving. This is in fact the standard convention for the classical stable homotopy category.
We leave it to the reader to unravel what the differential $d_2$ corresponds to under this identification. The remainder of this subsection is devoted to proving \autoref{E_*W_s's_acyclic_resolution_of_E_*Y} and \autoref{E_1_page_line_resolution_identification}. To start, we establish the following convention:

\begin{definition}\label{nu^n_isos}
	Given some (nonnegative integer) $n\geq0$, define natural isomorphisms $\nu^n_X:\Sigma^\n X\to\Sigma^nX$ inductively, by setting $\nu^0_X:=\lambda_X$, $\nu^1_X:=\nu_X^{-1}$, and supposing $\nu^{n-1}_X$ has been defined for some $n>1$, define $\nu^n_X$ to be the composition
...
\end{proposition}
\begin{proof}
	By \autoref{W_s_cellular_if_E_and_Y_are}, since $E$ and $Y$ are cellular, $W_t$ is as well for each $t\geq0$. Furthermore, for $t>0$, we have isomorphisms
	...
	\[0\to E_*(W_0)\xr{E_*(\delta_0)}E_*(\Sigma W_1)\xr{E_*(\delta_1)}E_*(\Sigma^2W_2)\to\cdots\to E_*(\Sigma^sW_s)\to\cdots\]
	actually lives in $E_*(E)\text-\CoMod^A$, as desired. Now, let $t\geq0$, and consider the following diagram:
	...
	is an isomorphism is entirely analagous. Thus, for each $t\geq0$, we have constructed isomorphisms
...
In this subsection, we briefly sketch some converge properties of the spectral sequence. Boardman already works quite generally in \cite{Boardman_1999}, so most of this is simply a review of the material contained within. From now on, we assume familiarity with derived limits of ($A$-graded) abelian groups (see Boardman \S1), filtered ($A$-graded) groups (see Boardman \S2), convergence of spectral sequences (Boardman Definition 5.2) and conditional convergence of a spectral sequence associated to an unrolled exact couple (Boardman Definition 5.10). We adopt his notation, writing
\[E_\infty^s(X,Y):=\(\bigcap_{r=1}^\infty Z_r^s\)/\(\bigcup_{r=1}^\infty B_r^s\)\qquad\text{and}\qquad RE_\infty(X,Y):=\Rlim_rZ_r^s\]
to denote the $E_\infty$-term and the derived $E_\infty$-term of the spectral sequence, respectively.

Ideally, the $E$-Adams spectral sequence for ${[X,Y]}_*$ would give us information which allows us to compute the group ${[X,Y]}_*$. Note that ${[X,Y]}_*$ is the colimit of the unrolled exact couple which determines the spectral sequence, as $Y_s=Y$ for $s<0$. Furthermore, since $(E_r(X,Y),d_r)$ is a half-plane spectral sequence with entering differentials, we may apply the results from \cite[\S7]{Boardman_1999}, where suitable conditions under which the spectral sequence converges to the colimit ${[X,Y]}_*$ are described (in particular, see Theorem 7.3 there). Unfortunately, in practice, the conditions outlined there are not usually satisfied for this spectral sequence, namely, in order for the spectral sequence to converge to ${[X,Y]}_*$, we must have that $\lim_s{[X,Y_s]}_*=0$. There is no reason to believe this would be satisfied, so we must take an alternative approach. Following Section 5 of Bousfield's seminal paper \cite{Bousfield_79}, we can instead set up the spectral sequence by means of a tower \emph{under} $Y$. First, we must define the \emph{$E$-nilpotent completion of $Y$}:
%
...
\begin{definition}[{\cite[pgs.\ 272--273]{Bousfield_79}}]\label{defn:nilpotent_completion}
	...
	The triangles from (\ref{quotient_triangle_Y's}) for $s\geq0$ may be spliced together to yield a tower $\{Y/Y_{s}\}_s$ under $Y$:
	...
	\end{tikzcd}\]
	where here the dashed arrows are really (degree $-\1$) maps $Y/Y_s\to\Sigma W_s$. The fact that this is a tower under $Y$ follows from diagram (\ref{gross_octa_diagram_Y's}), which tells us that $Y\xr b Y/Y_s$ factors as $Y\xr bY/Y_{s+1}\xr qY/Y_s$. We define the \emph{$E$-nilpotent completion of $Y$} to be the object $Y^\wedge_E$ (defined up to non-canonical isomorphism) obtained as the homotopy limit of this tower (\autoref{homotopy_limit_defn}):
	\[Y^\wedge_E:=\holim_sY_s/Y.\]
	Since $Y^\wedge_E$ is the homotopy limit of a tower under $Y$, it comes equipped with a canonical map $Y\to Y^\wedge_E$.
\end{definition}

\begin{remark}
	In \cite{Bousfield_79}, the $E$-nilpotent completion of $Y$ is denoted ``$E^\wedge Y$'', while the notation ``$Y^\wedge_E$'' we use here is standard in the modern literature.
\end{remark}
...
\begin{proposition}
	...
	\[\begin{tikzcd}
		...
		\arrow["r", dashed, from=1-3, to=2-3]
	\end{tikzcd}\]
	We may extend it to the right by defining $Y/Y_s=W_s=0$ for $s<0$. Then by \autoref{X,Y*_LES_compact}, we may apply the functor ${[X,-]}_*$ which yields the following $A$-graded unrolled exact couple:
	...
	\end{tikzcd}\]
	Thus by \cite[\S0]{Boardman_1999}, there is an induced spectral sequence. This spectral sequence is precisely the $E$-Adams spectral sequence for ${[X,Y]}_*$ (\autoref{ASS}).
\end{proposition}
\begin{proof}
	Let $(E'_r(X,Y),d_r')$ denote this new spectral sequence. For $s\geq0$, define
	...
	We claim these maps ${(f_s,g_s)}_s$ define a homomorphism of $A$-graded unrolled exact couples between the unrolled exact couple given above determined by the quotient tower $\{Y/Y_s\}$ under $Y$, and that obtained by applying ${[X,-]}_*$ to the canonical $E$-Adams resolution, i.e., that the following diagram commutes for all $s\in\bZ$:
	...
	Thus, we have defined a homomorphism of $A$-graded unrolled exact couples, and it is straightforward to check that therefore the maps $g_s$ lift to well-defined graded homomorphisms $\wt g_r^s:E_r^s(X,Y)\to {E_r^s}'(X,Y)$ for $s\geq0$ sending a class $[x]\in Z_r^s/B_r^s=E_r^s(X,Y)$ to the class $\wt g_r^s([x]):=[g_s(x)]$ in ${Z_r^s}'/{B_r^s}'={E_r^s}'(X,Y)$, which make the following diagrams commute for all $r\geq1$:
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{E_r(X,Y)} & {E_r'(X,Y)} && {\ker d_r} & {\ker d_r'} \\
		{E_r(X,Y)} & {E_r'(X,Y)} && {E_{r+1}(X,Y)} & {E_{r+1}'(X,Y)}
		\arrow["{\wt g_r}", from=1-1, to=1-2]
		\arrow["{d_r'}", from=1-2, to=2-2]
		\arrow["{d_r}"', from=1-1, to=2-1]
		\arrow["{\wt g_r}", from=2-1, to=2-2]
		\arrow["{\wt g_r}", from=1-4, to=1-5]
		\arrow[two heads, from=1-5, to=2-5]
		\arrow[two heads, from=1-4, to=2-4]
		\arrow["{\wt g_{r+1}}", from=2-4, to=2-5]
	\end{tikzcd}\]
	(commutativity of the first diagram implies the top arrow in the second diagram is well-defined). Yet we know that each $g_s$ is the identity, so that we shown that $(E_r(X,Y),d_r)=(E_r'(X,Y),d_r')$, as desired.
\end{proof}

By means of this new presentation of the spectral sequence, we may consider the sense in which the spectral sequence converges to the \emph{limit} $\lim_s{[X,Y/Y_s]}_*$ of the tower $\{Y/Y_s\}_s$ under $Y$, by means \cite[Theorem 7.4]{Boardman_1999}. First of all, it is standard that since $Y^\wedge_E$ is the homotopy limit of this tower, we have a \emph{Milnor short exact sequence}
\[0\to\Rlim_s{[X,Y/Y_s]}_{*+1}\to{[X,Y^\wedge_E]}_*\to\lim_s{[X,Y/Y_s]}_*\to 0\]
(the same argument given in \cite[Theorem 4.9]{Boardman_1999} works, although we warn the reader that Boardman has a sign error there --- he writes the first term in the short exact sequence with $-1$, when it should be $+1$). Thus, if $\Rlim_s{[X,Y/Y_s]}_*$ vanishes, we get an identification of the limit
\[\lim_s{[X,Y/Y_s]}_*={[X,Y^\wedge_E]}_*.\]
By \cite[Theorem 7.4]{Boardman_1999}, this is further satisfied if the derived $E_\infty$-term $RE_\infty(X,Y)$ is zero, in which case the spectral sequence converges strongly to the limit ${[X,Y^\wedge_E]}_*$, meaning in particular the natural maps
\begin{gather*}
