...
In light of this remark, when working in a tensor triangulated category with sub-Picard grading, we will usually simply write $\phi$ or even just $\cong$ for any isomorphism that is built by taking compositions of products of $\phi_{a,b}$'s, unitors, associators, identities, and their inverses.

In \cite{Dugger_2014}, Dugger studied the more general notion of an additive symmetric monoidal category $(\cC,\otimes,S)$ equipped with an abelian group $A$ and a group homomorphism $h:A\to\Pic(\cC)$. In particular, Dugger explored whether or not given a chosen representative $S^a$ in each isomorphism class $h(a)$ we can find such a coherent family of isomorphisms $\phi_{a,b}:S^{a+b}\xr\cong S^a\otimes S^b$ for $a,b\in A$. The answer, given in Section 7 of Dugger's paper, is that we can always find such a coherent family, although it is certainly not unique, nor is there a canonical choice for such a family (see Dugger's Proposition 7.1). Furthermore, given such a coherent family of isomorphisms, if we define $\pi_*(S)$ to be the $A$-graded abelian group $\pi_*(S):=\bigoplus_{a\in A}[S^a,S]$, we may endow it with an associative and unital graded product sending $x:S^a\to S$ and $y:S^b\to S$ to the composition
...
The upshot of this discussion is that given a tensor triangulated category, in order to give it a sub-Picard grading, all one needs to do is give the information specified in the first two bullet points in \autoref{sub_Picard_grading_defn}, and then one gets a coherent family of isomorphisms for free, although they must make a choice between several different and non-canonical choices of such families. Furthermore, as we will see in \Cref{section:monoid_in_SH}, this ring structure on $\pi_*(S)$ directly controls essentially all of the additional algebraic structure we can place on hom-groups of objects in $\cSH$, so one must be very careful to choose the ``correct'' family. 

\subsection{The category \texorpdfstring{$\cSH$}{SH} and its conventions}

Now, we will fix the category in which we will work for the remainder of this document. First, recall the notion of \emph{compact objects} in a category, which in an additive category may be characterized by the following simplified definition:
...
The motivating examples of such a category are the following:\begin{itemize}
	...
	\[h:RO(G)\to\Pic(\mbf{ho}G\mbf{Sp})\]
	taking a representation $V$ to the representation sphere $S^V$.
\end{itemize}

For a treatment of the classical stable homotopy category and its properties, we refer the reader to the nLab page \cite{nlab:introduction_to_stable_homotopy_theory}, which gives the construction in explicit detail. For the motivic stable homotopy category, we refer the reader to the wonderful treatment given in Section 2 of the paper \cite{WilOst} by Wilson and {\O}stv{\ae}r. There the construction and properties are only reviewed, and no proofs are given, but at the beginning of the section a comprehensive list of resources is given.
...
In order to reinforce our idea of $\cSH$ as ``a stable homotopy category'', we would like to establish some relevant notational conventions in $\cSH$. Given
