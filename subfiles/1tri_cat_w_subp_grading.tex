...
where $e_{X,Y}:\Sigma X\otimes Y\to\Sigma(X\otimes Y)$ is the isomorphism specified by the fact that $\cSH$ is tensor-triangulated. The first two arrows are natural in $X$ by definition. The last arrow is natural in $X$ by functoriality of $-\otimes-$. By abuse of notation, we will also use $\nu$ to denote this natural isomorphism. 
...
\subsection{The homotopy long exact sequence}
...
\begin{proof}
	...
	We may further define $s^0$ to be the identity natural transformation on $\cSH$. Then we get the following natural isomorphisms of $A$-graded abelian groups for all $n\in\bZ$
...
\begin{remark}\label{X,Y*_LES_compact}
	Expressed more compactly, the above proposition says that each object $W$ and distinguished triangle
	\[X\xr fY\xr gZ\xr h\Sigma X\]
...
\begin{proof}
	...
	where the first arrow is an isomorphism, per our assumption that $f$ is an isomorphism. Now consider the following diagram:
...
Many of the proofs in this section are quite technical and not very euclidiating, so we direct the reader to the appendix for most proofs. Monoid objects in $\cSH$ will be the focus of the rest of this paper. For a review of monoid objects in a symmetric monoidal category, see \Cref{monoid_objects_subsection_appendix}. The most important example of a monoid object in $\cSH$ is the unit $S$, which has multiplication map $\phi_{0,0}^{-1}=\lambda_S=\rho_S:S\otimes S\to S$ and unit map $\id_S:S\to S$.
...
\begin{definition}[\autoref{R-GrCAlg_defn}]\label{pi*S-commutative_ring}
	An \emph{$A$-graded $\pi_*(S)$-commutative ring} is a pair $(R,\varphi)$, where $R$ is an $A$-graded ring and $\varphi:\pi_*(S)\to R$ is an $A$-graded ring homomorphism such that for all homogeneous $x,y\in R$,
...
We should think of objects in $\GrCAlg{\pi_*(S)}$ as ``$A$-graded (anti)commutative rings''. A useful fact is that this category has binary coproducts and pushouts (\autoref{R-GrCAlg_has_pushouts_and_binary_coproducts}). As one would expect, every commutative monoid object $(E,\mu,e)$ in $\cSH$ gives rise to an $A$-graded $\pi_*(S)$-commutative ring $\pi_*(E)$:
...
\begin{corollary}\label{pi_*(E)_and_E_*(E)_are_pi_*(S)-commutative_rings}
    Let $(E,\mu,e)$ be a commutative monoid object in $\cSH$. Then by \autoref{pi_*:CMon_SH-->pi_*(S)-GrCAlg_main}, $\pi_*(E)$ and $E_*(E)=\pi_*(E\otimes E)$ are canonically $A$-graded $\pi_*(S)$-commutative rings (\autoref{pi*S-commutative_ring}), since $E\otimes E$ is a commutative monoid object in $\cSH$ by \autoref{product_of_monoids_is_monoid}.
\end{corollary}
