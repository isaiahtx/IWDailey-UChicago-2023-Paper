...
%\begin{definition}\label{defn_karoubi_complete}
%	A category $\cC$ is \emph{Karoubi complete} (also \emph{Cauchy complete} or \emph{idempotent complete}) if every idempotent splits in $\cC$. Explicitly, an \emph{idempotent} in $\cC$ is an endomorphism $e:X\to X$ such that $e\circ e=e$, and it \emph{splits} if there exists some object $Y$ and morphisms $i:Y\to X$, $r:X\to Y$ such that $r\circ i=\id_Y$ and $i\circ r=e$.
%\end{definition}
...
\begin{proof}
	...
	where the first isomorphism follows by additivity of $\Sigma^n$ and $\Sigma^{-n}=(S^{-\1}\otimes-)^n$ (since $\cSH$ is tensor triangulated, so $-\otimes-$ is additive in each argument).
	...
	\end{tikzcd}\]
	Thus the diagonals exhibit isomorphisms between $0$ and $W$, as desired.
\end{proof}
...
\begin{remark}\label{X,Sigma^aY*_eq_X,Y*-a}
	For all objects $X$, $Y$ in $\cSH$ and $a\in A$, there are isomorphisms
	\[{[X,\Sigma^aY]}_*\cong{[X,Y]}_{*-a},\]
	natural in both $X$ and $Y$, given by the compositions
	\[{[X,\Sigma^aY]}_*=[S^*\otimes X,S^a\otimes Y]\cong[S^{-a}\otimes S^*\otimes X,Y]\xr{(\phi_{-a,*}\otimes X)_*}[S^{*-a}\otimes X,Y]={[X,Y]}_{*-a},\]
	where the unlabelled isomorphism is the adjunction \autoref{Sigma^a,Sigma^-a_adjoint_equiv_main}.
\end{remark}
