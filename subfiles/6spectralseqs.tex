...
\begin{definition}[The spectral sequence associated to an unrolled exact couple]\label{SSeq_assoc_to_unrolled_EC}
    ...
    Expressed differently, we have constructed a $\bZ\times A$-graded spectral sequence $r\geq1\mapsto(E_r,d_r)$, where $E_r:=\bigoplus_{s\in\bZ}E_r^s$ and the differentials
    \[d_r:E_r\to E_r\]
    have $\bZ\times A$-degree $(r,\deg j-(r-1)\cdot\deg i+\deg k)$.
\end{definition}
