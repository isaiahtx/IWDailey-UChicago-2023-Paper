...
\begin{definition}
    A \textit{strongly tensor triangulated category} $(\cC,\otimes,S,S^1,S^{-1},\omega,\cD)$ is the data of: 
    \begin{itemize}
        \item An additive symmetric monoidal category $(\cC,\otimes,S)$,
        \item A pair of distinguished invertible elements $S^1$ and $S^{-1}$ in $\cC$ along with a ,
        \item A collection $\cD$ of \emph{triangles} in $\cC$:
        \[X\to Y\to Z\to S^1\otimes X.\]
    \end{itemize}
    These data must satisfy the following conditions:
    \begin{itemize}
        \item $-\otimes-$ is additive in each variable,
        \item 
    \end{itemize}
\end{definition}
