...
\begin{definition}\label{tentri}
    A \textit{tensor triangulated category} is a triangulated symmetric monoidal category $(\cC,\otimes,S,\Sigma,\Omega,\cD)$ such that:\begin{enumerate}[label=\textbf{TT\arabic*}]
        ...
        \[X\otimes C\xrightarrow{X\otimes h}X\otimes(\Sigma A)\xrightarrow{e_{X,A}}\Sigma(X\otimes A)\]
        and
        \[C\otimes X\xrightarrow{h\otimes X}(\Sigma A)\otimes X\xrightarrow{\tau}X\otimes(\Sigma A)\xrightarrow{e_{X,A}}\Sigma(X\otimes A)\xrightarrow{\Sigma\tau}\Sigma(A\otimes X),\]
        respectively.
    \end{enumerate}
\end{definition}
...
\begin{proof}
    ...
    \[\Sigma S\otimes X\xrightarrow{\tau}X\otimes\Sigma S\xrightarrow{e_{X,S}}\Sigma(X\otimes S)\xrightarrow{\Sigma\rho_X}\Sigma X,\]
    where $\rho$ is the right unitor specified by the monoidal structure on $\cC$.
\end{proof}

Because of the above proposition, when working with tensor triangulated categories we will often assume that $\Sigma=S^1\otimes-$ for some object $S^1$.
