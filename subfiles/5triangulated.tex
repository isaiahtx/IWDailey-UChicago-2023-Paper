...
\begin{definition}\label{triangulated_defn}
    A \emph{triangulated category} $(\cC,\Sigma,\cD)$ is the data of:\begin{enumerate}
        ...
        \item A collection $\cD$ of \emph{distinguished} triangles in $\cC$, where a is a sequence of arrows of the form
        \[X\to Y\to Z\to\Sigma X.\]
...
It turns out that the above definition is actually redundant; TR3 and TR4 follow from the remaining axioms (see Lemmas 2.2 and 2.4 in \cite{MayTri}). From now on, we fix a triangulated category $(\cC,\Sigma,\cD)$. To start, recall the following definition:
...
\begin{proof}
    ...
    Thus, we have shown exactness of 
    \[[\Sigma X,A]\xr{h^*}[Z,A]\xr{g^*}[Y,A]\xr{f^*}[X,A]\]
    at $[Y,A]$. To see $\ker g^*=\imm h^*$, again the same arguments applied to the shifted triangle
    \[Y\xr gZ\xr h\Sigma X\xr{-\Sigma f}\Sigma Y\]
    yield that
    \[[\Sigma Y,A]\xr{(-\Sigma f)^*}[\Sigma X,A]\xr {h^*}[Z,A]\xr{g^*}[Y,A]\]
    is exact at $[Z,A]$, so $\ker g^*=\imm h^*$, as desired.
\end{proof}
...
\end{proposition}
\begin{proof}
    Since $\Sigma$ is an equivalence, there exists some functor $\Omega:\cC\to\cC$ and natural isomorphisms $\eta:\Omega\Sigma\Rightarrow\Id_\cC$ and $\vare:\Id_\cC\Rightarrow\Sigma\Omega$. By axiom TR2, we have a distinguished triangle
...
\begin{definition}[{\cite[Definition 1.6.4]{Neeman_2001}}]
    Assume that $\cC$ has countable coproducts. Let
    \[X_0\xr{j_1}X_1\xr{j_2} X_2\xr{j_3}X_3\to\cdots\]
    be a sequence of objects and morphisms in $\cC$. The \emph{homotopy colimit} of the sequence, denoted $\holim X_i$, is by definition given, up to non-canonical isomorphism, as the cofiber of the map
    \[\coprod_{i=0}^\infty X_i\xr{1-\text{shift}}\coprod_{i=0}^\infty X_i,\]
    where the shift map $\coprod_{i=0}^\infty X_i\xr{\text{shift}}\coprod_{i=0}^\infty X_i$ is understood to be the direct sum of $j_{i+1}:X_i\to X_{i+1}$. In other words, the map $1-\text{shift}$ is the infinite matrix
    \[\begin{bmatrix}
        \id_{X_0} & 0 & 0 & 0 & \cdots \\
        -j_1 & \id_{X_1} & 0 & 0 & \cdots \\
        0 & -j_2 & \id_{X_2} & 0 & \cdots \\
        0 & 0 & -j_3 & \id_{X_3} & \cdots \\
        \vdots & \vdots & \vdots & \vdots & \ddots
    \end{bmatrix}\]
\end{definition}

\begin{proposition}[{\cite[Proposition 1.6.8]{Neeman_2001}}]\label{idempotent_splits_in_tri_cat_with_countable_coproducts}
...
\end{lemma}
\begin{proof}
    Note that unravelling definitions, then $\wt h$ and $\wt g$ are the compositions 
...
\begin{lemma}\label{semi_tensor_exact_sequence_remains_exact}
    Let $A\xr aB\xr bC\xr cD$ be any sequence isomorphic to a distinguished triangle. Then given any $E$ in $\cC$, the sequence
...
\begin{proof}
    ...
    % https://q.uiver.app/#q=
    \[\begin{tikzcd}[sep=scriptsize]
        ...
        \arrow["{e'_{E,X}}"{description}, from=1-7, to=2-6]
        \arrow["{E\otimes \delta}"{description}, from=2-6, to=3-7]
        \arrow["{(E\otimes\delta)\circ e'_{E,X}}", dashed, from=1-7, to=3-7]
    \end{tikzcd}\]
    The top triangle is distinguished by axiom TT3 for a tensor triangulated category, thus exact by \autoref{distinguished_tri_is_exact}, so that the bottom triangle is also exact since the vertical arrows are isomorphisms and each square commutes.
\end{proof}
...
\begin{proposition}\label{LES_remains_exact_after_tensor}
    Suppose we have a distinguished triangle
    \[X\to Y\to Z\to\Sigma X\]
    in $\cC$. Then given any object $E$ in $\cC$, the long exact sequence given in \autoref{dist_tri_LES} remains exact after applying $E\otimes-$.
\end{proposition}
\begin{proof}
